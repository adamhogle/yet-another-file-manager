...
- Emit nginx-style shaped lines for login outcomes and download outcomes as
  `tracing::info!` events under target `yafm::access`, so they flow through
  the existing stdout stream. Access logging is always on. The line is not a
  drop-in combined-log line: the event field carries an event name (`LOGIN`,
  `DOWNLOAD <path>`) instead of the HTTP request line, and the timestamp is
  RFC3339 instead of the CLF form, so classic combined-log consumers need a
  custom format (goaccess and fail2ban both support one). The user field is
  quoted because the display name is free-form, like the referer and
  user-agent.
...
- Log bytes as the promised Content-Length for 200/206 and 0 otherwise. A
  HEAD logs 0: axum routes HEAD to the GET handler with the body removed, so
  the handler promises a Content-Length while transferring nothing (nginx
  parity). A client that disconnects mid-stream still logs the promised
  length; counting bytes actually written would require wrapping the body
  stream and logging on completion, a bigger refactor with no current need.
...
- If the proxy does not sanitize `X-Forwarded-For`, a client can still inject
  ...
  trail and the field is bounded to 45 characters. Quotes, backslashes,
  C0/DEL/C1 controls and the Unicode line/paragraph separators in every
  user-controlled field are escaped at render time, so a forged path or
  header cannot split a log line.
