...
- Every critical line of code must have test coverage. Handlers, validation, auth and path normalization run under the Rust coverage gate, which fails CI below 99 percent library line coverage and on any new uncovered line. A new uncovered line is either tested or deliberately accepted into `backend/coverage-baseline.json` during review; see `docs/architecture/0008-rust-test-coverage-threshold.md`.
