...
- Every critical line of code must have test coverage. Handlers, validation, auth and path normalization run under the Rust coverage gate, which fails CI below 99 percent library line coverage and on any uncovered line the report lists that `backend/coverage-baseline.json` does not accept; a new uncovered line is either tested or deliberately accepted into the baseline during review. See `docs/architecture/0008-rust-test-coverage-threshold.md`.
- Verify UI changes by rendering them, not only with unit tests: rebuild the frontend and check every affected state at desktop, tablet and phone widths (for example 1440, 768, 390 and 320 px) against a server that serves real listing data. Breakpoint bugs such as overflowing columns, unreachable actions and panels of inconsistent width do not show up in vitest; capture full-page screenshots per width so before and after states can be compared.
